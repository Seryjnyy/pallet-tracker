import {
    ...
    deleteItemType,
    getItemTypeFromRef,
} from "@/db/services/item-type-service";
...
export default function ItemType() {
    const { data } = useLiveQuery(getItemTypeFromRef());
    const [isPending, startTransition] = useTransition();
    const [newItemType, setNewItemType] = useState("");

    const onSubmit = async () => {
        if (isPending) return;
        if (newItemType.length == 0) return;
        if (newItemType.length > 100) return;

        startTransition(() => {
            addItemType(newItemType);
            setNewItemType("");
        });
    };
    ...
    return (
        ...
        >
            ...
            >
                <Input
                    onChangeText={(newVal) => setNewItemType(newVal)}
                    value={newItemType}
                />
